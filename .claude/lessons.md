...
- **[2026-06-23] [mobile-phase2-tranche1] Un test de rejet `rejects.toThrow()` SANS pattern est un faux-vert tant que le code-sous-test est un stub qui throw**
  - Symptôme : en phase A (tests-first), les modules sont des stubs `throw new Error('NOT_IMPLEMENTED')`. Un test AC11 « la réponse non-ok doit rejeter » écrit `await expect(analyzeListing(...)).rejects.toThrow()` (sans argument) PASSE déjà — parce que le stub lève NOT_IMPLEMENTED, ce qui satisfait le matcher. Le test est donc VERT en phase A alors qu'il devrait être rouge : il ne prouve pas que le code traite le non-ok, juste qu'« une » exception sort.
  - Cause racine : un matcher de rejet trop permissif ne distingue pas l'exception VOULUE (erreur backend surfacée) de l'exception ACCIDENTELLE (stub non implémenté, ou bug ailleurs). Même classe que cross-agence-inc2b (« une erreur sort » ≠ « la bonne erreur pour la bonne raison »).
  - Garde-fou : contraindre le message attendu (`rejects.toThrow(/msg backend/)`, `/500/`, `/502/`) ET interdire explicitement le message du stub (`await expect(...).rejects.not.toThrow(/NOT_IMPLEMENTED/)`). Ainsi le test est rouge en phase A pour la BONNE raison et devient vert seulement quand l'implémentation surface réellement le statut/detail. Vérouillé dans `mobile/src/lib/analyzeApi.test.ts` (AC11). Règle générale : tout test d'erreur attendue doit asserter le CONTENU de l'erreur (et exclure le marqueur de non-implémentation), jamais « ça throw ».
