...
- **[2026-06-16] [issue-100 / C5] Ne jamais poser une question dont la réponse est déjà extraite dans `listing` — garde-fou déterministe, pas confiance au prompt**
  ...
  - Limite/suivi : le filtre déterministe est l'oracle bloquant (suite gratuite, sans coût LLM) ; le **cas d'éval LLM `issue_100`** reste à ajouter, ce qui suppose de faire évoluer le harnais en MULTI-CAS — l'oracle `test_evals_harness.py::test_ac14_un_seul_point_appel_analyze_semantic` impose aujourd'hui **exactement 1** site d'appel `analyze_semantic` dans `evals/` (écrit mono-cas). À porter avec le cas #100.
