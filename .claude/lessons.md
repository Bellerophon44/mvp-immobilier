...
- **[2026-06-25] [mobile-phase2-distribution] `eas update` n'utilise PAS le `env` de `eas.json` — sans `.env` à l'export, `EXPO_PUBLIC_*` est vide → « Network request failed »**
  - Symptôme : l'app publiée par `eas update` (aperçu Expo Go) échouait à l'analyse avec « Network request failed », alors que `/health` du backend répondait et que `eas.json` portait bien `EXPO_PUBLIC_API_URL`. Le bundle appelait une URL vide.
  - Cause racine : deux chemins d'injection distincts. `eas build` applique le `build.<profil>.env` de `eas.json` ; **`eas update` ne le fait pas** — il inline les `EXPO_PUBLIC_*` depuis le `.env` / l'environnement **au moment de l'export Metro**. Le repo cloné n'avait pas de `.env` (gitignoré), donc variable absente → `getApiUrl()` renvoie `''` → `fetch('/analyze')` (URL relative) rejette en natif. Aggravant : Expo Go sert parfois l'ancien bundle en cache (faux « ça ne change pas »).
  - Garde-fou : avant tout `eas update`, garantir `EXPO_PUBLIC_API_URL` (créer le `.env`, ou exporter la variable) ; le log doit afficher `env: load .env` + `env: export EXPO_PUBLIC_API_URL`. Diagnostic : « Network request failed » natif = URL vide/invalide (souvent bundle sans env ou ancien bundle en cache), à distinguer d'un backend down (vérifier `/health` d'abord). À fiabiliser via variables d'env **côté serveur Expo** (`eas env:create` + `eas update --environment <profil>`) pour ne plus dépendre du `.env` local. Documenté dans `mobile/README.md`.

- **[2026-06-25] [atelier / process] Une branche coupée du défaut (`main`) qui est EN AVANCE sur `staging` rend la PR `feature→staging` en conflit (`dirty`) — merger `main` dans la branche d'abord**
  - Symptôme : la PR `feature → staging` ressortait `mergeable_state: dirty` (donc CI muette, cf. leçon 2026-06-18) avec ~14k additions, alors que le travail était mobile + un fichier frontend.
  - Cause racine : les sessions Claude Code web coupent la branche depuis la branche **par défaut** (`main`), or `staging` est **en retard** sur `main`. La branche divergeait d'un ancêtre commun en deçà de `staging` → 3-way merge conflictuel sur les fichiers touchés des deux côtés (`page.tsx`, `lessons.md`).
  - Garde-fou : pour le flux staging-first quand la branche vient de `main`, **merger `origin/main` dans la branche et résoudre une fois** ; la branche devient alors un sur-ensemble de `main` ⊇ `staging`, donc `feature→staging` puis `staging→main` passent sans nouveau conflit (fast-forward). Règle : vérifier `git merge-base`/`rev-list` branche vs `main` vs `staging` avant d'ouvrir la PR ; ne pas confondre « PR dirty » avec un problème de budget/CI.

- **[2026-06-25] [mobile-phase2-distribution] iOS : aucun build cloud gratuit sur iPhone réel — compte Apple Developer (99 $/an) obligatoire ; le PWA est le seul chemin iPhone 0 €**
  - Symptôme : plan « build de dev 7 jours gratuit avant de payer » irréalisable sur iPhone via EAS (Windows, pas de Mac).
  - Cause racine : toute install sur iPhone réel (dev build, internal, TestFlight) exige des credentials Apple gérés via le Developer Program payant ; le « 7 jours » d'Apple n'existe qu'en signature locale Xcode (Mac). Android, lui, produit un APK installable gratuitement.
  - Garde-fou (règle produit/process) : proposer d'emblée les bons chemins par plateforme — **Android = APK EAS gratuit** ; **iPhone gratuit = PWA** (`coherence-metz.fr` installable, mais sans extraction WebView ni partage natif) ; **iPhone natif = Apple Developer payant**. Piège d'inscription (France) : la vérif d'identité Apple refuse souvent le **permis** (« pas valide pour la région ») → fournir **passeport/CNI**, via l'app iPhone *Apple Developer*, compte en région France.

- **[2026-06-27] [mobile-tranche-b-historique] Whitelist anti-contenu-tiers (§11.3) : récursive en profondeur, ET dérogation par CLÉ exprimée en ALLOW POSITIF de chemin — jamais blacklist de position**
  - Symptôme : la whitelist de l'historique local (interdit `raw_text`/`image_urls`/`photos`/`body`/`text` à tout niveau, sauf `claims[].text`) a échoué deux fois en boucle adversariale. (1) Testeur B : le contrôle n'inspectait que la RACINE de `result` → un `raw_text`/`image_urls` IMBRIQUÉ (`result.local_context.*`) fuyait dans la sortie réellement stockée (et rechargé par `parseRecords`). (2) Reviewer : après passage en récursif, `text` était traité en blacklist de POSITION (interdit seulement à la racine) → un `text` brut sous `local_context.text` ou `pillars[].text` passait verbatim, alors que la seule dérogation prévue est `claims[].text`.
  - Cause racine : (1) contrôle non récursif ; oracle AC2 d'origine n'injectait les champs interdits qu'à la racine → faux-vert. (2) dérogation `claims[].text` implémentée par EXCLUSION d'un emplacement (racine) au lieu d'AUTORISATION du seul chemin légitime (`claims[]`). Même classe que 9.10 (whitelist positive vs blacklist), cross-agence-inc2b (« absence ≠ testée en profondeur ») et contexte-local-v2 (« invariant sur CHAQUE niveau atteignable »).
  - Garde-fou : détection RÉCURSIVE consciente du chemin dans `mobile/src/lib/history.ts` — `raw_text`/`image_urls`/`photos`/`body` interdits à toute profondeur ; `text` autorisé UNIQUEMENT quand son parent direct est un élément du tableau `claims` (drapeau `parentIsClaimsItem` propagé dans la récursion, remis à faux sous tout autre conteneur), interdit partout ailleurs. Appliquée à `buildRecord` (lève `ForbiddenFieldError` nommée) ET `parseRecords`/`isValidRecord` (entrée empoisonnée rejetée sans exception). Tests `mobile/src/lib/history.hardening.test.ts` AXE1/AXE2 (clés profondes) + AXE7 (`text` sous `local_context`/`pillars[]`/`claims[].nested` interdit, `claims[].text` conservé) : prouvent l'absence par inspection RÉCURSIVE de la SORTIE STOCKÉE réelle (`JSON.stringify` du store in-memory), sonde matchant une CLÉ JSON (`"x":`) pas une sous-chaîne. Règle : (a) un invariant « champ interdit à aucun niveau » s'oracle par injection EN PROFONDEUR et preuve d'absence dans l'artefact réellement persisté ; (b) une dérogation par clé s'exprime comme un allow POSITIF du chemin exact, jamais comme une exclusion de position.

- **[2026-06-27] [mobile-tranche-b-historique] Tri stable à clé d'ordre ÉGALE : `push` + sort ne respecte pas « le nouveau passe en tête » — `unshift` avant le tri stable**
  - Symptôme : `upsertRecord` faisait `push(record)` puis tri stable décroissant par `savedAt` ; à `savedAt` ÉGAL (deux URLs distinctes au même ms), l'entrée fraîchement upsertée restait DERRIÈRE l'existante, contredisant la spec §5.1 (« à savedAt égal, l'entrée qui vient d'être upsertée passe en tête »). Aucun test d'origine ne couvrait l'égalité de clé de tri → faux-vert (trou de couverture sur la borne d'égalité).
  - Cause racine : un tri STABLE préserve l'ordre d'insertion sur les ex æquo ; `push` met le nouveau en dernier, donc il reste dernier parmi les ex æquo. Borne d'égalité non testée (parenté avec « bornes aux valeurs exactes » 9.7 appliquée à l'ordre).
  - Garde-fou : `unshift(record)` (nouveau en tête) AVANT le tri stable décroissant → l'upserté gagne les ex æquo. Test `history.hardening.test.ts` AXE8 (deux URLs même `savedAt` → la dernière upsertée en index 0 ; re-upsert dédup même `savedAt` → remonte en tête). Règle : quand une spec impose « le plus récent / le nouveau d'abord » et que la clé de tri peut être à égalité, tester explicitement l'égalité et garantir l'ordre voulu (ne pas se reposer sur la stabilité du tri seule).
