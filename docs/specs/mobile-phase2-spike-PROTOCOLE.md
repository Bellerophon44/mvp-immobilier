...
### Niveau 2 — WebView in-app (Expo Go, iPhone, 2026-06-23) : ✅ CONCLUANT
Mini-app `spikes/lbc-extraction/` lancée sur un iPhone réel via Expo Go (SDK 54),
sur le réseau de l'utilisateur (pas un datacenter) :
- ✅ **La WebView charge LeBonCoin sur device sans captcha** — accueil ET page
  d'annonce réelle. Le mur DataDome ne se déclenche pas pour un vrai appareil/
  navigateur, exactement comme en desktop.
- ✅ **L'injection JS fonctionne in-app** : `injectedJavaScript` +
  `window.ReactNativeWebView.postMessage` → l'aller-retour DOM → natif marche.
- ✅ **Galerie récupérée** : sur une vraie annonce, `ad-large` > 0 + liste de la
  galerie (le filtre desktop tient sur mobile). Sur l'accueil : `ad-large = 0`,
  `ad-image`/`pp-small` uniquement (cohérent : pas de galerie hors annonce).
- **Finding (intent de partage)** : le partage natif iOS fournit du **texte +
  URL** (« Voici une annonce … sur leboncoin: https://… »), pas une URL nue.
  Charger la phrase entière fait échouer la WebView (« outside the sandbox »).
  L'app doit **extraire la 1ʳᵉ URL http(s)** du payload partagé (implémenté dans le
  spike via `firstUrl()`). À reprendre tel quel dans la spec (share intent).
- **Lazy-load** : l'utilisateur (ou l'app) doit faire défiler la galerie pour que
  toutes les photos soient dans le DOM avant extraction.

## 10. Verdict du spike : ✅ FAISABILITÉ PROUVÉE (Niveaux 1 + 2)

L'extraction on-device (texte + URLs de galerie) est **viable** pour LeBonCoin en
**React Native + Expo (WebView)**. Le risque technique n°1 (Q3 de l'analyse) est
**levé**. Techno tranchée par le résultat : **RN/Expo** (Q2-a). Inputs pour la
spec de l'app Tier 1 (GATE 2) :
1. **Extraction texte** : `document.body.innerText` (complet).
2. **Galerie** : ne garder que `rule=ad-large`, dédup par chemin d'image, exclure
   `ad-image` (annonces similaires), `bo-*` (logos), `pp-small` (promos). Robustesse
   à terme : scoper au conteneur DOM de la galerie plutôt qu'au nom de `rule`.
3. **Share intent** : extraire la 1ʳᵉ URL http(s) du texte partagé.
4. **Lazy-load** : auto-scroller la galerie avant collecte.
5. **Contrat backend** : POSTer `raw_text` (le texte) + `image_urls` (la galerie)
   à `/analyze` — déjà livré en Phase 1, rien à faire côté backend.
6. **RGPD** : extraction sur la seule annonce ouverte par l'utilisateur, pas de
   stockage, URLs jamais loggées (déjà garanti serveur).
